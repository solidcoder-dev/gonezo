...
internal class AndroidMovementShareRepository(private val database: CoreDatabase) : MovementShareRepository {
  ...
  private fun mapShare(c: android.database.Cursor): MovementShare {
    ...
    return MovementShare(id, c.getString(c.getColumnIndexOrThrow("source_transaction_id")), SharingPersonId.from(c.getString(c.getColumnIndexOrThrow("payer_person_id"))), BigDecimal(c.getString(c.getColumnIndexOrThrow("total_amount"))), c.getString(c.getColumnIndexOrThrow("currency")), participants, Instant.parse(c.getString(c.getColumnIndexOrThrow("created_at"))), Instant.parse(c.getString(c.getColumnIndexOrThrow("updated_at"))))
  }
}
...
internal class AndroidPlannedMovementShareRepository(private val database: CoreDatabase) : PlannedMovementShareRepository {
  ...
  private fun find(where: String, args: Array<String>): PlannedMovementShare? = database.readableDatabase.query("sharing_planned_expense_shares", null, where, args, null, null, null, "1").use { c -> if (!c.moveToFirst()) null else { val id = PlannedMovementShareId(UUID.fromString(c.getString(c.getColumnIndexOrThrow("id")))); val participants = database.readableDatabase.query("sharing_planned_expense_share_participants", null, "planned_share_id = ?", arrayOf(id.toString()), null, null, "participant_order asc").use { rows -> buildList { while (rows.moveToNext()) add(PlannedMovementShareParticipant(PlannedMovementShareParticipantId(UUID.fromString(rows.getString(rows.getColumnIndexOrThrow("id")))), SharingPersonId.from(rows.getString(rows.getColumnIndexOrThrow("person_id"))), rows.getString(rows.getColumnIndexOrThrow("participant_parts"))?.toInt(), BigDecimal(rows.getString(rows.getColumnIndexOrThrow("amount"))), rows.getInt(rows.getColumnIndexOrThrow("reimbursable")) == 1, rows.getInt(rows.getColumnIndexOrThrow("participant_order")))) } }; PlannedMovementShare(id, ExpectedMovementRef(c.getString(c.getColumnIndexOrThrow("expected_movement_ref"))), c.getString(c.getColumnIndexOrThrow("source_plan_id"))?.let { RecurringSharePlanId(UUID.fromString(it)) }, SharingPersonId.from(c.getString(c.getColumnIndexOrThrow("payer_person_id"))), RecurringShareAllocationMode.from(c.getString(c.getColumnIndexOrThrow("mode"))), c.getString(c.getColumnIndexOrThrow("payer_parts"))?.toInt(), BigDecimal(c.getString(c.getColumnIndexOrThrow("total_amount"))), c.getString(c.getColumnIndexOrThrow("currency")), participants, PlannedMovementShareStatus.valueOf(c.getString(c.getColumnIndexOrThrow("status")).uppercase()), c.getString(c.getColumnIndexOrThrow("materialized_transaction_ref")), c.getString(c.getColumnIndexOrThrow("materialized_share_ref"))?.let(MovementShareId::from), Instant.parse(c.getString(c.getColumnIndexOrThrow("created_at"))), Instant.parse(c.getString(c.getColumnIndexOrThrow("updated_at")))) } }
  private fun ContentValues.putNullable(key: String, value: String?) { if (value == null) putNull(key) else put(key, value) }
}
