...
internal class AndroidExpenseShareRepository(private val database: CoreDatabase) : ExpenseShareRepository {
  ...
  override fun listAll(): List<ExpenseShare> = database.readableDatabase.query("sharing_expense_shares", arrayOf("source_transaction_id"), null, null, null, null, "id asc").use { c -> buildList { while (c.moveToNext()) findBySourceTransactionId(c.getString(0))?.let(::add) } }
...
internal class AndroidPlannedExpenseShareRepository(private val database: CoreDatabase) : PlannedExpenseShareRepository {
  ...
  override fun listAll(): List<PlannedExpenseShare> = database.readableDatabase.query("sharing_planned_expense_shares", arrayOf("id"), null, null, null, null, "id asc").use { c -> buildList { while (c.moveToNext()) findById(PlannedExpenseShareId(UUID.fromString(c.getString(0))))?.let(::add) } }
