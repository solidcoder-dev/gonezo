...
export function FlowTabView({ required, provided }: FlowTabViewProps) {
  ...
  return <div className={styles.stack} aria-busy={required.status.loading}>
    <section className={styles.card} aria-label="Flow summary">
      <div className={styles.summary}><div className={styles.metric}><span>{report.summary.openingLabel}</span><FinancialAmountView formattedAmount={report.summary.opening} visibility={visibility} /></div><div className={styles.metric}><span>{report.summary.endLabel}</span><FinancialAmountView formattedAmount={report.summary.end} visibility={visibility} /></div><div className={styles.metric}><span>Lowest point</span><FinancialAmountView formattedAmount={report.summary.lowest} visibility={visibility} className={styles.negative} /><span className={styles.supporting}>on {report.summary.lowestDate}</span></div></div>
      ...
      <div className={styles.upcoming}><div className={styles.upcomingItem}><span className={styles.icon} aria-hidden><i className="bi bi-arrow-down-left" /></span><div><FinancialAmountView formattedAmount={report.upcoming.incoming} visibility={visibility} /><span className={styles.supporting}>Upcoming money in · {report.upcoming.incomingText}</span></div></div><div className={`${styles.upcomingItem} ${styles.expense}`}><span className={styles.icon} aria-hidden><i className="bi bi-arrow-up-right" /></span><div><FinancialAmountView formattedAmount={report.upcoming.outgoing} visibility={visibility} /><span className={styles.supporting}>Upcoming money out · {report.upcoming.outgoingText}</span></div></div></div>
    </section>
    <section className={styles.section} aria-label="Flow insights"><h2>Flow insights</h2><div className={styles.insights}>{report.insights.map((item) => <article className={`${styles.insight} ${styles[item.tone]}`} key={item.key}><span className={styles.icon} aria-hidden><i className={item.icon} /></span><div className={styles.insightText}><strong>{item.title}</strong><span className={styles.supporting}>{item.supportingText}</span><FinancialAmountView formattedAmount={item.amount} visibility={visibility} className={styles.amount} /></div></article>)}</div></section>
  </div>;
}
