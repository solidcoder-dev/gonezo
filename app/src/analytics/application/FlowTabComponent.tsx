...
export type FlowTabComponentProps = { required: { context: { core: AnalyticsPort }; config: { enabled: boolean; currency: string; filters?: AnalyticsFiltersInput; refreshSignal: boolean; amountVisibility?: AmountVisibility } }; provided?: { events?: { onError?: (error: { message: string }) => void } } };
...
export function FlowTabComponent({ required, provided }: FlowTabComponentProps) {
  const { core } = required.context; const { enabled, currency, filters, refreshSignal, amountVisibility } = required.config;
  ...
  return <div className={styles.stack}><FlowTabView required={{ report: enabled && currency ? report : undefined, status: { loading: enabled && Boolean(currency) && Boolean(core.analyticsGetFlowReport) && loadedKey !== requestKey, error: !core.analyticsGetFlowReport ? 'Flow report is unavailable' : error?.key === requestKey ? error.message : undefined, amountVisibility } }} provided={{ state: { canPrevious: report?.canGoPrevious ?? true, canNext: report?.canGoNext ?? effectiveShift < 0 }, commands: { previous: () => { setSelectionKey(filterKey); setShift(() => effectiveShift - 1); }, next: () => { setSelectionKey(filterKey); setShift(() => Math.min(0, effectiveShift + 1)); } } }} /></div>;
}
