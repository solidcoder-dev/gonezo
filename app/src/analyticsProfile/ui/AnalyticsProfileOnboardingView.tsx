...
function ProfileFields({ draft, update, regions, errors }: {
  ...
  return <>
    <Field id="birthYear" label="Year of birth" error={errors.birthYear}><input id="birthYear" type="number" inputMode="numeric" min="1900" max={new Date().getFullYear()} value={draft.birthYear || ''} aria-invalid={Boolean(errors.birthYear)} aria-describedby={errors.birthYear ? 'birthYear-error' : undefined} onChange={(event) => update('birthYear', Number(event.target.value))} /></Field>
