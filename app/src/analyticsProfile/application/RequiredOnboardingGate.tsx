...
export function RequiredOnboardingGate({ port, children }: { port: AnalyticsProfilePort; children: ReactNode }) {
  ...
  const loadProfile = useCallback(async () => {
    try {
      const profile = await getAnalyticsProfile(port, userId);
      setState(isAnalyticsProfileComplete(profile)
        ? { status: 'complete', profile }
        : { status: 'required', profile, error: '' });
    } catch {
      setState({ status: 'error', message: 'Profile could not be loaded. Try again.' });
    }
  }, [port, userId]);
  ...
  if (state.status === 'error') return <main className="analytics-profile-status"><p role="alert">{state.message}</p><button type="button" onClick={() => { setState({ status: 'loading' }); void loadProfile(); }}>Try again</button></main>;
