...
export function AnalyticsContributionConsentGate({ port, clock, children }: {
  ...
  useEffect(() => {
    ...
    void getContributionConsent(port, userId)
      .then((consent) => {
        if (active) setState(consent ? { status: 'complete', consent } : { status: 'undecided' });
      })
      .catch(() => {
        if (active) setState({ status: 'error', message: 'Contribution choice could not be loaded. Try again.' });
      });
    return () => { active = false; };
