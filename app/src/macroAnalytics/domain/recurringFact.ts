...
export type RecurringFactInput = RecurringFact;

export function createRecurringFact(input: RecurringFactInput): RecurringFact {
  ...
  if (!/^(?:0|[1-9]\d*)(?:\.\d+)?$/.test(input.amount) || /^0(?:\.0+)?$/.test(input.amount)) {
    throw new Error('Recurring fact amount must be a positive decimal string');
  }
