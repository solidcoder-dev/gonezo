...
export function ShareExpenseEditorView({ required, provided }: ShareExpenseEditorViewProps) {
  ...
  return <div className={`${styles.shareEditor} d-flex flex-column flex-grow-1`}>
    <div className="flex-grow-1 d-grid gap-4">
      ...
      <SegmentedControlView<ShareMode> required={{ config: { ariaLabel: 'Share mode', columns: 2 }, data: { options: [{ value: 'parts', label: 'As parts' }, { value: 'amounts', label: 'As amounts' }] }, state: { value: model.state.mode }, status: { disabled } }} provided={{ commands: { select: model.commands.selectMode } }} />
      <section aria-label="Share people" className="d-grid gap-2"><div className="d-flex justify-content-between text-secondary small"><span>Participants</span><span>{model.state.people.length}</span></div><ul className="list-unstyled m-0 p-0 d-grid" aria-label="Share people">{model.state.people.map((person) => { const participant = isParticipant(person); return <li key={person.id} className={`${styles.personRow} d-grid gap-2 py-3`}><div className="d-flex align-items-start gap-3 min-w-0"><span className={`${styles.avatar} ${AVATAR_TONES[person.avatarTone]}`} aria-hidden="true">{person.role === 'owner' ? 'You' : person.name.slice(0, 1)}</span><div className="d-grid min-w-0 flex-grow-1"><strong className="text-truncate">{person.name}</strong>{participant ? <ShareSettlementDropdownView name={person.name} movementType={model.state.movementType} value={person.settlementChoice} disabled={disabled} onChange={(choice: ShareSettlementChoice) => model.commands.updateSettlement(person.id, choice)} /> : <span className="text-secondary small">Payer</span>}</div><div className="d-flex align-items-start gap-2">{model.state.mode === 'parts' ? <div className="d-grid justify-items-center"><div className={styles.partsStepper}><button type="button" aria-label={`Decrease parts for ${person.name}`} disabled={disabled || person.parts <= 1} onClick={() => model.commands.updateParts(person.id, person.parts - 1)}><i className="bi bi-dash-lg" aria-hidden="true" /></button><input aria-label={`${person.name} parts`} type="number" min="1" step="1" value={person.parts} onChange={(event) => model.commands.updateParts(person.id, Number(event.target.value))} inputMode="numeric" /><button type="button" aria-label={`Increase parts for ${person.name}`} disabled={disabled} onClick={() => model.commands.updateParts(person.id, person.parts + 1)}><i className="bi bi-plus-lg" aria-hidden="true" /></button></div><span className="text-secondary small">{person.amount} {currency}</span></div> : <label className={`${styles.amountField} d-flex align-items-center`}><span className="visually-hidden">{person.name} amount</span><input aria-label={`${person.name} amount`} type="number" min="0" step="0.01" value={person.amount} onChange={(event) => model.commands.updateAmount(person.id, event.target.value)} inputMode="decimal" placeholder="0.00" disabled={disabled} /><span className="text-secondary small">{currency}</span></label>}{participant ? <button type="button" className={`gz-icon-button ${styles.removePersonButton}`} aria-label={`Remove ${person.name}`} disabled={disabled} onClick={() => model.commands.removePerson(person.id)}><i className="bi bi-trash" aria-hidden="true" /></button> : <span className={styles.personSpacer} aria-hidden="true" />}</div></div></li>; })}</ul></section>
