...
export function SegmentedControlView<TValue extends string>({
  ...
  return (
    <div className={`${styles.control} ${required.config.columns === 2 ? styles.twoColumns : ''} ${required.config.variant === 'quiet' ? styles.quiet : ''} ${required.config.compact ? styles.compact : ''}`} role="radiogroup" aria-label={required.config.ariaLabel}>
