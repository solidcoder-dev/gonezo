...
export function BalanceProjectionChartView({ currentMarkerAt, lowestAt, domain, ticks, currentMarkerLabel, points }: Props) {
  ...
  return <ResponsiveContainer width="100%" height={220}><LineChart data={data} margin={{ top: 12, right: 8, bottom: 0, left: -12 }}><CartesianGrid vertical={false} stroke="rgba(32,32,30,.08)" /><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fill: '#74746b', fontSize: 11 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: '#74746b', fontSize: 11 }} ticks={chartTicks} domain={chartDomain} tickFormatter={(value) => formatExactAxisValue(Number(value), '')} width={48} /><Tooltip formatter={(value) => [`${Number(value).toFixed(2)}`, 'Balance']} /><ReferenceLine x={marker} stroke="rgba(32,32,30,.2)" strokeDasharray="4 4" /><ReferenceLine x={lowest?.label} stroke="#e23b30" strokeDasharray="2 2" /><Line type="monotone" dataKey="postedBalance" stroke="#2563eb" strokeWidth={2} dot={false} connectNulls /><Line type="monotone" dataKey="projectedBalance" stroke="#2563eb" strokeWidth={2} dot={{ r: 3 }} strokeDasharray="6 5" connectNulls /></LineChart></ResponsiveContainer>;
}
