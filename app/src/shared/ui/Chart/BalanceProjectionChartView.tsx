...
export function BalanceProjectionChartView({ currentMarkerAt, lowestAt, domain, ticks, currentMarkerLabel, points }: Props) {
  ...
  return <ResponsiveContainer width="100%" height={220}><LineChart data={data} margin={{ top: 12, right: 8, bottom: 0, left: -12 }}><CartesianGrid vertical={false} stroke="var(--color-border-subtle)" /><XAxis dataKey="key" ticks={axisTicks} interval={0} axisLine={false} tickLine={false} tick={{ fill: 'var(--color-text-muted)', fontSize: 11 }} tickFormatter={(value) => data.find((point) => point.key === value)?.label ?? ''} /><YAxis axisLine={false} tickLine={false} tick={{ fill: 'var(--color-text-muted)', fontSize: 11 }} ticks={chartTicks} domain={chartDomain} tickFormatter={(value) => formatExactAxisValue(Number(value), '')} width={48} /><Tooltip formatter={(value) => [`${Number(value).toFixed(2)}`, 'Balance']} /><ReferenceLine x={markerKey} stroke="var(--color-border-strong)" strokeDasharray="4 4" /><ReferenceLine x={lowest?.key} stroke="var(--color-expense)" strokeDasharray="2 2" /><Line type="monotone" dataKey="postedBalance" stroke="var(--color-transfer)" strokeWidth={2} dot={false} connectNulls /><Line type="monotone" dataKey="projectedBalance" stroke="var(--color-transfer)" strokeWidth={2} dot={{ r: 3 }} strokeDasharray="6 5" connectNulls /></LineChart></ResponsiveContainer>;
}
