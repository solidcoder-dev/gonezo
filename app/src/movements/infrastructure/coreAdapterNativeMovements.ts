...
import type { TaxonomyListCategoriesResult } from '../../taxonomy/application/taxonomyCore.port';
...
export async function searchNativeMovements(
  ...
  if (input.source === 'expected') {
    const [result, categories]: [Awaited<ReturnType<CorePort['expectedListMovements']>>, TaxonomyListCategoriesResult] = await Promise.all([
