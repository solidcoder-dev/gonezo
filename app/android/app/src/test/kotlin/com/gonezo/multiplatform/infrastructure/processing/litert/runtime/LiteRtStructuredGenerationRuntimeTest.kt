...
class LiteRtStructuredGenerationRuntimeTest {
  ...
  fun `source delegates backend selection and avoids CPU or multimodal fallbacks`() {
    val source = java.nio.file.Path.of("../../../platforms/android/infrastructure/src/main/java/com/gonezo/multiplatform/infrastructure/processing/litert/runtime/LiteRtStructuredGenerationRuntime.kt").readText()
  ...
  private fun modelConfiguration(target: MlExecutionTarget = MlExecutionTarget.GPU): InterpretationModelConfiguration = InterpretationModelConfiguration(
    ...
    target = target,
    npuTarget = if (target == MlExecutionTarget.NPU) com.gonezo.multiplatform.infrastructure.ml.NpuTarget.QUALCOMM_SM8850 else null,
  )
