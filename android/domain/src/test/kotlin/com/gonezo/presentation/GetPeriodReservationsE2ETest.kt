...
@Testcontainers
class GetPeriodReservationsE2ETest {
  ...
  fun setup() {
    flyway.migrate()
    val resource = resourceLoader.getResource("classpath:sql/get_period_reservations_setup.sql")
  ...
  fun `returns active reservations for period`() {
    ...
    val response = restClient.get()
      ...
      .retrieve()
      .toEntity(Array<BudgetReservationResponse>::class.java)

    val body = response.body!!.toList()
    val rows = jdbcTemplate.queryForList(
      "select id, status from budget_reservations where budget_period_id = ? and status = 'active'",
      periodId,
    )

    assertThat(body).hasSize(rows.size)
    val responseIds = body.map { it.id.toString() }.toSet()
    val dbIds = rows.map { it["id"].toString() }.toSet()
    assertThat(responseIds).isEqualTo(dbIds)
  }
