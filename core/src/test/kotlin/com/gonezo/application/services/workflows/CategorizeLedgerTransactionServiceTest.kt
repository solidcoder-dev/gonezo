...
class CategorizeLedgerTransactionServiceTest {
    ...
    fun `throws when orchestration processing ends as failed`() {
        ...
        val createCategoryUC = RecordingCreateCategoryUC()
        val processUC =
            RecordingProcessCategorizationUC(
...
private class InMemoryCategoryRepository(private vararg val categories: Category) : CategoryRepository {
    override fun save(category: Category) = Unit

    override fun findById(id: CategoryId): Category? = categories.firstOrNull { it.id == id }

    override fun findByIds(ids: Collection<CategoryId>): Map<CategoryId, Category> = ids.mapNotNull { id -> categories.firstOrNull { it.id == id }?.let { id to it } }.toMap()

    override fun findByNormalizedNameAndAppliesTo(name: String, appliesTo: CategoryAppliesTo): Category? = categories.firstOrNull { it.name.equals(name.trim(), ignoreCase = true) && it.appliesTo == appliesTo }

    override fun listAll(): List<CategoryWithUsage> = categories.map { CategoryWithUsage(it, 0) }
}

private class RecordingCreateCategoryUC(private val generatedId: CategoryId = CategoryId.random()) : CreateCategoryUC {
...
private class RecordingProcessCategorizationUC(private val forcedStatus: CategorizationStatus = CategorizationStatus.ASSIGNED, private val errorCode: String? = null, private val errorMessage: String? = null) : ProcessTransactionCategorizationUC {
