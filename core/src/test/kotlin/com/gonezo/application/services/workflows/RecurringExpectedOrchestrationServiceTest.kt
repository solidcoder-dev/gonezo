...
class RecurringExpectedOrchestrationServiceTest {
    ...
    fun `due recurring event creates pending expected movement`() {
        ...
        val service =
            HandleRecurringMovementDueForExpectedService(
    ...
    fun `automatic recurring event is not projected as expected`() {
        ...
        val event =
            recurringDueEvent(occurrenceId = UUID.randomUUID().toString()).copy(
    ...
    fun `approving expected created from recurrence posts ledger resolves expected and acknowledges occurrence`() {
        ...
        val service =
            ApproveRecurringExpectedMovementService(
                expectedMovementRepository = expectedRepository,
    ...
    fun `dismiss expected created from recurrence dismisses expected and acknowledges occurrence failure as user dismissed`() {
        ...
        val service =
            DismissRecurringExpectedMovementService(
                expectedMovementRepository = expectedRepository,
    ...
    private fun recurringDueEvent(occurrenceId: String, movementType: String = "expense", amount: String = "10.00", currency: String = "USD", categoryId: String? = null): RecurringMovementDueIntegrationEvent = RecurringMovementDueIntegrationEvent(
    ...
    private class FixedRecordLedgerExpenseUC(private val txId: String) : RecordLedgerExpenseUC {
    ...
    private class ThrowingRecordLedgerIncomeUC(private val message: String) : RecordLedgerIncomeUC {
        override fun execute(command: RecordLedgerIncomeCommand): TransactionId = throw IllegalStateException(message)
    }
}
