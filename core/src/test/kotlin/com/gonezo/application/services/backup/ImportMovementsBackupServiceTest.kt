...
class ImportMovementsBackupServiceTest {
    ...
    private fun minimalBackupSnapshot(): MovementsBackupSnapshot = MovementsBackupSnapshot(
        schemaVersion = 2,
        exportedAt = Instant.parse("2026-05-10T12:00:00Z"),
        accounts =
        listOf(
...
private class MutableLedgerTransactionRepository : LedgerTransactionRepository {
    ...
    override fun findByAccountAndPeriod(accountId: AccountId, range: DateRange): List<Transaction> = values.values.filter { it.accountId == accountId && it.occurredAt >= range.from && it.occurredAt <= range.to }

    override fun findByAccountAndMerchant(accountId: AccountId, merchant: String): List<Transaction> = values.values.filter { it.accountId == accountId && it.merchant.equals(merchant, ignoreCase = true) }
}
...
private class MutableTagRepository : TagRepository {
    ...
    override fun findByIds(ids: Collection<TagId>): Map<TagId, Tag> = ids.mapNotNull { id -> values[id]?.let { id to it } }.toMap()

    override fun findByNormalizedName(name: String): Tag? = values.values.firstOrNull { it.name.equals(name.trim(), ignoreCase = true) }
...
private class MutableTransactionCategoryAssignmentRepository : TransactionCategoryAssignmentRepository {
    ...
    override fun findByTransactionIds(transactionIds: Collection<UUID>): Map<UUID, TransactionCategoryAssignment> = values.filterKeys(transactionIds::contains)
}
