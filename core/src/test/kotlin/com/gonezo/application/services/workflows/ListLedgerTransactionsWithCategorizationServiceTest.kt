...
class ListLedgerTransactionsWithCategorizationServiceTest {
    ...
    private fun serviceWith(transactions: List<Transaction>, categories: List<Category> = emptyList(), assignments: List<TransactionCategoryAssignment> = emptyList(), states: List<TxCategorizationState> = emptyList()): ListLedgerTransactionsWithCategorizationService = ListLedgerTransactionsWithCategorizationService(
...
private class InMemoryLedgerTransactionRepository(private val transactions: List<Transaction>) : LedgerTransactionRepository {
    ...
    override fun findByAccount(accountId: AccountId, limit: Int?): List<Transaction> = transactions.filter { it.accountId == accountId }.let { list -> limit?.let { list.take(it) } ?: list }

    override fun findByAccountAndPeriod(accountId: AccountId, range: DateRange): List<Transaction> = transactions.filter {
        it.accountId == accountId &&
            !it.occurredAt.isBefore(range.from) &&
            !it.occurredAt.isAfter(range.to)
    }

    override fun findByAccountAndMerchant(accountId: AccountId, merchant: String): List<Transaction> = transactions.filter { it.accountId == accountId && it.merchant.equals(merchant, ignoreCase = true) }
}

private class InMemoryAssignmentRepository(assignments: List<TransactionCategoryAssignment>) : TransactionCategoryAssignmentRepository {
    ...
    override fun findByTransactionIds(transactionIds: Collection<UUID>): Map<UUID, TransactionCategoryAssignment> = transactionIds.mapNotNull { id -> values[id]?.let { id to it } }.toMap()
}

private class InMemoryCategoryRepository(categories: List<Category>) : CategoryRepository {
    ...
    override fun findByIds(ids: Collection<CategoryId>): Map<CategoryId, Category> = ids.mapNotNull { id -> values[id]?.let { id to it } }.toMap()

    override fun findByNormalizedNameAndAppliesTo(name: String, appliesTo: CategoryAppliesTo): Category? = values.values.firstOrNull { it.name.equals(name.trim(), ignoreCase = true) && it.appliesTo == appliesTo }

    override fun listAll(): List<CategoryWithUsage> = values.values.map { CategoryWithUsage(it, 0) }
}

private class InMemoryStateRepository(states: List<TxCategorizationState>) : TxCategorizationStateRepository {
    ...
    override fun findByTransactionIds(transactionIds: Collection<UUID>): Map<UUID, TxCategorizationState> = transactionIds.mapNotNull { id -> values[id]?.let { id to it } }.toMap()

    override fun deleteByTransactionIds(transactionIds: Collection<UUID>) {
        transactionIds.forEach(values::remove)
    }

    override fun findPending(now: Instant, limit: Int): List<TxCategorizationState> = values.values.filter { it.status == CategorizationStatus.PENDING }.take(limit)
}
