...
private class InMemoryTagRepository(private vararg val tags: Tag) : TagRepository {
    override fun save(tag: Tag) = Unit

    override fun findById(id: TagId): Tag? = tags.firstOrNull { it.id == id }

    override fun findByIds(ids: Collection<TagId>): Map<TagId, Tag> = ids.mapNotNull { id -> tags.firstOrNull { it.id == id }?.let { id to it } }.toMap()

    override fun findByNormalizedName(name: String): Tag? = tags.firstOrNull { it.name.equals(name.trim(), ignoreCase = true) }
...
private class InMemoryTagAssignmentRepository : TransactionTagAssignmentRepository {
    ...
    override fun findByTransactionId(transactionId: UUID): List<TransactionTagAssignment> = values[transactionId] ?: emptyList()

    override fun findByTransactionIds(transactionIds: Collection<UUID>): Map<UUID, List<TransactionTagAssignment>> = transactionIds.mapNotNull { id -> values[id]?.let { id to it } }.toMap()
}
