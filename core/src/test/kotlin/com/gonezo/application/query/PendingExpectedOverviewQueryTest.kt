...
class PendingExpectedOverviewQueryTest {
    ...
    fun `separates pending expenses and incomes without mixing currencies`() {
        ...
        assertThat(result.expenses.amountsByCurrency)
            .extracting<String> { it.currency }
    ...
    private fun service(preferredCurrency: String?, rows: List<PendingExpectedMovementRead>) = GetPendingExpectedOverviewService(
        object : PendingExpectedOverviewReadPort {
            override fun read() = rows
        },
    )

    private fun row(type: String, currency: String, amount: String, count: Int = 1) = List(count) { PendingExpectedMovementRead(type, currency, amount) }
}
