...
class MovementReuseTemplateQueryTest {
    ...
    fun `returns reusable data without historical values`() {
        val expected = MovementReuseTemplateRead(
            "movement-1", "Mercadona", "main", "Main", "expense",
            ...
            MovementReuseTemplateDetails("25.00", listOf(MovementReuseTemplateItem("Fruit", "25.00")), listOf(MovementReuseTemplateShare("Alex", "25.00", true))),
        )
        ...
        assertThat(result!!.details!!.items).containsExactly(MovementReuseTemplateItem("Fruit", "25.00"))
        assertThat(result.details.sharing).containsExactly(MovementReuseTemplateShare("Alex", "25.00", true))
    }

    @Test
    fun `allows an empty details projection when movement has no items or sharing`() {
        val expected = MovementReuseTemplateRead("movement-1", "Plain", "main", "Main", "expense", null, emptyList(), emptyList(), emptyList(), null, false, null)
        val result = MovementReuseTemplateQueryService(object : MovementReuseTemplateReadPort {
            override fun readTemplate(representativeMovementId: String) = expected.takeIf { it.movementId == representativeMovementId }
        }).get(MovementReuseTemplateQuery("movement-1"))
