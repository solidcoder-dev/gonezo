...
class AnalyticsWindowResolverTest {
  ...
  fun `winter and summer days use zone starts rather than twenty four hour arithmetic`() {
    val summer = AnalyticsWindowResolver(clock, zone).resolve(
      AnalyticsPeriodSelection(AnalyticsPeriodKind.CUSTOM, LocalDate.of(2026, 3, 29), LocalDate.of(2026, 3, 29)), true,
    ).current!!
    val winter = AnalyticsWindowResolver(clock, zone).resolve(
      AnalyticsPeriodSelection(AnalyticsPeriodKind.CUSTOM, LocalDate.of(2026, 10, 25), LocalDate.of(2026, 10, 25)), true,
    ).current!!
