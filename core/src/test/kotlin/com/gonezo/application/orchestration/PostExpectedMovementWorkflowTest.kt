...
class PostExpectedMovementWorkflowTest {
    ...
    private fun fixture(): Fixture {
        ...
        val recordExpense = object : RecordLedgerExpenseUC {
            override fun execute(command: RecordLedgerExpenseCommand): TransactionId = TransactionId.random().also {
                fixture.recordedTransactions.add(it)
                fixture.recordExpenseCommands.add(command)
            }
        }
        val recordIncome = object : RecordLedgerIncomeUC {
            override fun execute(command: RecordLedgerIncomeCommand): TransactionId = TransactionId.random().also {
                fixture.recordedTransactions.add(it)
                fixture.recordIncomeCommands.add(command)
            }
        }
        ...
        fixture.workflow = PostExpectedMovementWorkflow(
            ...
            createExpenseDraft = object : CreateLedgerExpenseDraftUC {
            ...
            postDraft = object : PostLedgerDraftTransactionUC {
                override fun execute(command: PostLedgerDraftTransactionCommand) {
                    fixture.postedDrafts.add(command)
                }
            },
            categorize = object : CategorizeLedgerTransactionUC {
                override fun execute(command: CategorizeLedgerTransactionCommand) = command.also { fixture.categorizedCommands.add(it) }.let { TxCategorizationState(it.transactionId.value, it.categoryId, CategorizationStatus.NONE, null, null, 0, null, it.requestedAt, it.requestedAt) }
            },
            applyTags = object : ApplyTransactionTagsUC {
                override fun execute(command: ApplyTransactionTagsCommand) = ApplyTransactionTagsResult(emptyList())
            },
            ...
            projectNext = object : ExpectedOccurrenceProjectionService {
                override fun projectNext(recurringMovementId: String, projectedAt: Instant) = fixture.nextExpectedMovementId
            },
    ...
    private class InMemoryIdempotency : ExpectedPostingIdempotencyRepository {
        ...
        override fun save(processed: ProcessedExpectedPosting) {
            values[processed.idempotencyKey] = processed
        }
    }
