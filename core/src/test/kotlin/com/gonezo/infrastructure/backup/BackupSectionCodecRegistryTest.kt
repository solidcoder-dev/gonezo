...
class BackupSectionCodecRegistryTest {
    ...
    fun `root codec delegates to a registered replacement codec without changes to root`() {
        ...
        val registry = BackupSectionCodecRegistry(
            listOf(
        ...
        val document = ApplicationBackupJsonCodec(registry).decode(
            ApplicationBackupJsonCodec(registry).encode(
                com.gonezo.application.backup.contract.ApplicationBackupDocument(
                    "gonezo-backup",
                    1,
