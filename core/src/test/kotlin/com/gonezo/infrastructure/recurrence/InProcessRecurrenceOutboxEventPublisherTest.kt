...
class InProcessRecurrenceOutboxEventPublisherTest {
    ...
    fun `publishes recurring due event to expected handler`() {
        ...
        val uc =
            object : HandleRecurringMovementDueForExpectedUC {
        ...
        val publisher =
            InProcessRecurrenceOutboxEventPublisher(
    ...
    fun `throws on unsupported event type`() {
        val uc =
            object : HandleRecurringMovementDueForExpectedUC {
                override fun execute(command: HandleRecurringMovementDueForExpectedCommand): HandleRecurringMovementDueForExpectedResult = HandleRecurringMovementDueForExpectedResult(
                    expectedMovementId = ExpectedMovementId.random(),
                    created = true,
                )
            }
