...
class JdbcExpectedPostingIdempotencyRepositoryE2ETest : SqliteE2ETest() {
    ...
    private fun seedPostingRows() {
        db.namedJdbcTemplate.update(
            "insert into ledger_accounts(id, name, type, currency, status, created_at) values ('account-1', 'Cash', 'cash', 'EUR', 'active', '2026-07-20T10:00:00Z')",
            emptyMap<String, Any>(),
        )
        db.namedJdbcTemplate.update(
            "insert into expected_movements(id, account_id, movement_type, amount, currency, expected_at, status, created_at, updated_at) values ('expected-1', 'account-1', 'expense', '10.00', 'EUR', '2026-07-20T10:00:00Z', 'pending', '2026-07-20T10:00:00Z', '2026-07-20T10:00:00Z')",
            emptyMap<String, Any>(),
        )
        db.namedJdbcTemplate.update(
            "insert into ledger_transactions(id, account_id, type, amount, currency, occurred_at, status) values ('transaction-1', 'account-1', 'expense', '10.00', 'EUR', '2026-07-20T10:00:00Z', 'posted')",
            emptyMap<String, Any>(),
        )
        db.namedJdbcTemplate.update(
            "insert into ledger_transactions(id, account_id, type, amount, currency, occurred_at, status) values ('transaction-2', 'account-1', 'expense', '10.00', 'EUR', '2026-07-20T10:00:00Z', 'posted')",
            emptyMap<String, Any>(),
        )
    }
}
