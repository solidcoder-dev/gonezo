...
data class NotificationDeliveryResult(val submitted: Int, val suppressed: Int, val retried: Int, val cancelled: Int)

class DeliverPendingNotificationsService(private val notifications: NotificationRepository, private val deliveries: NotificationDeliveryQueue, private val sender: SystemNotificationSender) {
    fun execute(ownerId: String, now: Instant): NotificationDeliveryResult {
        ...
        val active = eligible.mapNotNull { delivery ->
            when (val result = notifications.findById(ownerId, delivery.notificationId)) {
                ...
                is NotificationLookupResult.Found -> if (result.row.notification.isUnreadActive) {
                    delivery
                } else {
                    deliveries.cancel(delivery.notificationId)
                    null
        ...
        if (active.isEmpty()) {
            if (notifications.countUnread(ownerId) == 0) {
                sender.removeSummary()
            } else if (sender.summaryExists()) {
                sender.updateSummary(notifications.countUnread(ownerId))
            }
