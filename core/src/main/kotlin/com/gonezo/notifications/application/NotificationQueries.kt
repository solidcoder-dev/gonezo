...
data class NotificationsListQuery(val filter: String, val beforeCursor: String? = null, val limit: Int = 30)

data class NotificationItem(val id: String, val type: String, val sourceType: String, val sourceId: String, val subject: String, val errorCode: String?, val occurredAt: Instant, val createdAt: Instant, val readAt: Instant?, val withdrawnAt: Instant?)

data class NotificationsListResult(val items: List<NotificationItem>, val nextCursor: String?, val snapshotCursor: String?)
...
class NotificationQueries(private val notifications: NotificationRepository, private val reconciliation: ReconcileNotificationsService? = null) {
    ...
    fun notificationsMarkRead(ownerId: String, id: String, at: Instant): NotificationCommandResult = notifications.markRead(ownerId, id, at).toCommandResult()

    fun notificationsMarkAllRead(ownerId: String, throughCursor: String, at: Instant): Int = notifications.markAllRead(ownerId, NotificationCursor.decode(throughCursor), at)

    fun notificationsReconcile(ownerId: String, relevance: NotificationRelevance, at: Instant): Int = requireNotNull(reconciliation) { "Notification reconciliation is not configured" }
