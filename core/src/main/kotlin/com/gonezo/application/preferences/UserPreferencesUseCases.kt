...
data class GetUserPreferencesQuery(val ownerId: PreferencesOwnerId = PreferencesOwnerId.LOCAL_USER)

interface GetUserPreferencesUC {
    fun execute(query: GetUserPreferencesQuery = GetUserPreferencesQuery()): UserPreferences
}

data class SetDefaultAccountCommand(val ownerId: PreferencesOwnerId = PreferencesOwnerId.LOCAL_USER, val accountId: DefaultAccountId)

interface SetDefaultAccountUC {
    fun execute(command: SetDefaultAccountCommand)
}

data class ClearDefaultAccountCommand(val ownerId: PreferencesOwnerId = PreferencesOwnerId.LOCAL_USER)
