...
data class AnalyticsMovementReadWindow(
  val fromInclusive: Instant,
  val toExclusive: Instant,
) {
  init { require(fromInclusive < toExclusive) { "analytics window must be non-empty" } }
...
object AnalyticsAmountResolver {
  fun resolve(fact: AnalyticsMovementFact, useFullAmount: Boolean): Money =
    if (useFullAmount) fact.fullAmount else fact.personalAmount
}

data class AnalyticsFinancialTotals(
  val income: Money,
  val expenses: Money,
  val netFlow: Money,
)

data class AnalyticsMovementReadResult(
  val window: AnalyticsMovementReadWindow,
  val facts: List<AnalyticsMovementFact>,
  val totals: AnalyticsFinancialTotals,
)

class AnalyticsMovementFactQuery(
  private val postedReader: AnalyticsPostedMovementReader,
  private val expectedReader: AnalyticsExpectedMovementReader,
  private val scheduledReader: AnalyticsScheduledMovementReader,
  private val factQuery: AnalyticsMovementFactQueryService = AnalyticsMovementFactQueryService(),
) {
  fun execute(
    window: AnalyticsMovementReadWindow,
    filters: AnalyticsMovementQueryFilters = AnalyticsMovementQueryFilters(),
    includePlannedMovements: Boolean = true,
  ): AnalyticsMovementReadResult {
    val facts = factQuery.query(
