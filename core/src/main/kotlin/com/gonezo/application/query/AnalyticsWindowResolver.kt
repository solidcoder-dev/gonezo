...
data class AnalyticsPeriodSelection(val kind: AnalyticsPeriodKind, val from: LocalDate? = null, val to: LocalDate? = null)
...
data class AnalyticsResolvedWindows(val current: AnalyticsInstantRange?, val comparison: AnalyticsInstantRange?)

class AnalyticsWindowResolver(private val clock: Clock, private val zoneId: ZoneId) {
    ...
    private fun currentDates(period: AnalyticsPeriodSelection, today: LocalDate, includePlannedMovements: Boolean): ClosedDateRange? = when (period.kind) {
        ...
        AnalyticsPeriodKind.ALL_TIME -> {
            null
        }
    }

    private fun comparisonDates(period: AnalyticsPeriodSelection, current: ClosedDateRange?, includePlannedMovements: Boolean): ClosedDateRange? {
        ...
        return when (period.kind) {
            AnalyticsPeriodKind.THIS_MONTH -> {
                ...
                if (includePlannedMovements) {
                    ...
                    ClosedDateRange(start, start.plusDays(current.days - 1))
                }
            }
            ...
            AnalyticsPeriodKind.THIS_YEAR -> {
                ...
                if (includePlannedMovements) {
                    ClosedDateRange(start, start.plusYears(1).minusDays(1))
                } else {
                    ClosedDateRange(start, start.plusDays(current.days - 1))
                }
            }
