...
class CreateExpectedMovementService(private val repository: ExpectedMovementRepository) : CreateExpectedMovementUC {
    override fun execute(command: CreateExpectedMovementCommand): ExpectedMovementId {
        val movement =
            ExpectedMovement.create(
...
class UpdateExpectedMovementService(private val repository: ExpectedMovementRepository) : UpdateExpectedMovementUC {
...
class ResolveExpectedMovementService(private val repository: ExpectedMovementRepository) : ResolveExpectedMovementUC {
...
class DismissExpectedMovementService(private val repository: ExpectedMovementRepository) : DismissExpectedMovementUC {
...
class ListExpectedMovementsService(private val repository: ExpectedMovementRepository) : ListExpectedMovementsUC {
    override fun execute(query: ListExpectedMovementsQuery): List<ExpectedMovementView> = repository.listByAccount(query.accountId, query.includeClosed).map { movement ->
...
private fun requireExpectedMovement(repository: ExpectedMovementRepository, id: ExpectedMovementId): ExpectedMovement = repository.findById(id)
