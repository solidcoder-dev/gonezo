...
class DefaultRecurringSharePlanService(private val plans: RecurringSharePlanRepository, private val people: SharingPersonRepository) : RecurringSharePlanService {
    override fun createOrUpdate(command: CreateRecurringSharePlanCommand): RecurringSharePlanId {
        ...
        val templates = command.participants.mapIndexed { index, input ->
            ...
            RecurringShareParticipantTemplate(
                id = RecurringShareParticipantTemplateId.random(),
                personId = person.id,
                parts = if (command.mode == RecurringShareAllocationMode.PARTS) {
                    require(input.parts != null && input.parts > 0) { "participant parts must be positive" }
                    input.parts
                } else {
                    null
                },
                fixedAmount = if (command.mode == RecurringShareAllocationMode.AMOUNTS) {
                    require(input.amount != null && input.amount > BigDecimal.ZERO) { "participant amount must be positive" }
                    input.amount
                } else {
                    null
                },
