...
class HandleRecurringMovementDueForExpectedService(private val createExpectedMovementUC: CreateExpectedMovementUC, private val expectedMovementRepository: ExpectedMovementRepository, private val expectedOccurrenceFactory: ExpectedOccurrenceFactory = DefaultExpectedOccurrenceFactory(), private val plannedShareInstantiator: PlannedShareInstantiator = NoOpPlannedShareInstantiator) : HandleRecurringMovementDueForExpectedUC {
    override fun execute(command: HandleRecurringMovementDueForExpectedCommand): HandleRecurringMovementDueForExpectedResult {
        ...
        require(
            command.event.movementType
                .trim()
                .lowercase() == ExpectedMovementType.EXPENSE.value ||
                command.event.movementType
                    .trim()
                    .lowercase() == ExpectedMovementType.INCOME.value,
        ) {
...
class ApproveRecurringExpectedMovementService(private val expectedMovementRepository: ExpectedMovementRepository, private val recordLedgerIncomeUC: RecordLedgerIncomeUC, private val recordLedgerExpenseUC: RecordLedgerExpenseUC, private val resolveExpectedMovementUC: ResolveExpectedMovementUC, private val acknowledgeOccurrenceUC: AcknowledgeRecurringMovementOccurrenceUC) : ApproveRecurringExpectedMovementUC {
    override fun execute(command: ApproveRecurringExpectedMovementCommand): ApproveRecurringExpectedMovementResult {
        ...
        val occurrenceId =
            movement.originOccurrenceId?.let(UUID::fromString)
...
class DismissRecurringExpectedMovementService(private val expectedMovementRepository: ExpectedMovementRepository, private val dismissExpectedMovementUC: DismissExpectedMovementUC, private val acknowledgeOccurrenceUC: AcknowledgeRecurringMovementOccurrenceUC) : DismissRecurringExpectedMovementUC {
...
private fun requireExpectedMovement(repository: ExpectedMovementRepository, expectedMovementId: com.gonezo.expected.domain.ExpectedMovementId): ExpectedMovement = repository.findById(expectedMovementId)
