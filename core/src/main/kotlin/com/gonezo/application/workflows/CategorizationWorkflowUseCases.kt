...
enum class CategorizationStatus(val value: String) {
    ...
    companion object {
        fun from(value: String): CategorizationStatus = entries.firstOrNull { it.value.equals(value, ignoreCase = true) }
            ?: throw IllegalArgumentException("Unsupported categorization status: $value")
    }
}

data class TxCategorizationState(val transactionId: UUID, val requestedCategoryId: CategoryId?, val status: CategorizationStatus, val errorCode: String?, val errorMessage: String?, val attempts: Int, val nextAttemptAt: Instant?, val updatedAt: Instant, val createdAt: Instant)
...
data class ProcessTransactionCategorizationCommand(val transactionId: UUID, val transactionType: String, val requestedCategoryId: CategoryId?, val processedAt: Instant)
