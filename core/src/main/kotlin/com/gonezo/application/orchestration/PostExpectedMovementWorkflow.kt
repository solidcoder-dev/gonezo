...
data class PostExpectedMovementCommand(val expectedMovementId: String, val movement: ExpectedPostingMovementSnapshot, val occurredAt: Instant, val categoryId: String? = null, val tagNames: List<String> = emptyList(), val ignored: Boolean = false, val sharingOverride: FinalPlannedShareDraft? = null, val idempotencyKey: String)

data class PostExpectedMovementResult(val transactionId: String, val shareId: String? = null, val nextExpectedMovementId: String? = null)

fun interface MovementIgnoredWriter {
    fun setIgnored(movementId: String, ignored: Boolean, changedAt: Instant)
}

data class ProcessedExpectedPosting(val idempotencyKey: String, val expectedMovementId: String, val result: PostExpectedMovementResult, val completionStatus: String = "completed")
