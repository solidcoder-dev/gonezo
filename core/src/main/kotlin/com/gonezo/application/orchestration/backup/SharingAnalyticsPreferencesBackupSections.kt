...
data class SharingBackupSection(val persons: List<BackupSharingPerson>, val expenseShares: List<BackupExpenseShare>, val recurringPlans: List<BackupRecurringSharePlan>, val plannedShares: List<BackupPlannedExpenseShare>) : BackupSection {
    ...
    override fun references() = persons.map { BackupReference.SharingPerson(it.id) }.toSet() +
        ...
        recurringPlans.map { BackupReference.RecurringSharePlan(it.id) } +
        plannedShares.map { BackupReference.PlannedExpenseShare(it.id) }
}

class SharingBackupSectionExporter(
    private val personRepository: SharingPersonRepository,
    private val expenseShareRepository: ExpenseShareRepository,
    private val recurringPlanRepository: RecurringSharePlanRepository,
    private val plannedShareRepository: PlannedExpenseShareRepository,
...
class SharingBackupSectionImporter(
    private val personRepository: SharingPersonRepository,
    private val expenseShareRepository: ExpenseShareRepository,
    private val recurringPlanRepository: RecurringSharePlanRepository,
    private val plannedShareRepository: PlannedExpenseShareRepository,
    ...
    override fun validate(section: BackupSection, context: BackupImportContext): BackupValidationResult {
        ...
        return try {
            ...
            section.expenseShares.forEach { share ->
                ...
                BigDecimal(share.totalAmount); Instant.parse(share.createdAt); Instant.parse(share.updatedAt)
            ...
            section.recurringPlans.forEach { plan ->
                ...
                RecurringShareAllocationMode.from(plan.mode); Instant.parse(plan.createdAt); Instant.parse(plan.updatedAt)
            ...
            section.plannedShares.forEach { share ->
                ...
                RecurringShareAllocationMode.from(share.mode); PlannedExpenseShareStatus.valueOf(share.status.uppercase()); BigDecimal(share.totalAmount); Instant.parse(share.createdAt); Instant.parse(share.updatedAt)
                share.participants.forEach { participant ->
    ...
    override fun import(section: BackupSection, context: BackupImportContext) {
        ...
        sharing.plannedShares.forEach { share ->
            plannedShareRepository.save(PlannedExpenseShare(PlannedExpenseShareId(UUID.fromString(share.id)), ExpectedMovementRef(share.expectedMovementId), share.sourcePlanId?.let { RecurringSharePlanId(UUID.fromString(it)) }, SharingPersonId.from(share.payerPersonId), RecurringShareAllocationMode.from(share.mode), share.payerParts, BigDecimal(share.totalAmount), share.currency, share.participants.map { PlannedExpenseShareParticipant(PlannedExpenseShareParticipantId(UUID.fromString(it.id)), SharingPersonId.from(it.personId), it.parts, BigDecimal(it.amount), it.reimbursable, it.order) }, PlannedExpenseShareStatus.valueOf(share.status.uppercase()), share.materializedTransactionId, share.materializedShareId?.let(ExpenseShareId::from), Instant.parse(share.createdAt), Instant.parse(share.updatedAt)))
        }
    }
}
...
class AnalyticsBackupSectionImporter(private val repository: AnalyticsExclusionRepository) : BackupSectionImporter {
    ...
    override fun validate(section: BackupSection, context: BackupImportContext): BackupValidationResult = try {
        ...
        section.exclusions.forEach {
            ...
            AnalyticsExclusionReason.from(it.reason); Instant.parse(it.createdAt)
        }
        BackupValidationResult.Valid
    } catch (error: IllegalArgumentException) { BackupValidationResult.Invalid(if (error is BackupReferenceValidationException) BackupErrorCode.INVALID_REFERENCE else BackupErrorCode.INVALID_DATA, error.message ?: "Invalid analytics backup section") }
    override fun import(section: BackupSection, context: BackupImportContext) {
...
class PreferencesBackupSectionImporter(private val repository: UserPreferencesRepository, private val ownerId: PreferencesOwnerId) : BackupSectionImporter {
    ...
    override fun validate(section: BackupSection, context: BackupImportContext): BackupValidationResult = try {
        ...
        BackupValidationResult.Valid
    } catch (error: IllegalArgumentException) { BackupValidationResult.Invalid(if (error is BackupReferenceValidationException) BackupErrorCode.INVALID_REFERENCE else BackupErrorCode.INVALID_DATA, error.message ?: "Invalid preferences reference") }
    override fun import(section: BackupSection, context: BackupImportContext) { repository.save(com.gonezo.preferences.domain.UserPreferences(ownerId, (section as PreferencesBackupSection).defaultAccountId?.let(DefaultAccountId::from))) }
}
