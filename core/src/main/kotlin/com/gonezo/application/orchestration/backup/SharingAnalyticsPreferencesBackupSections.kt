...
data class BackupMovementShare(val id: String, val sourceTransactionId: String, val payerPersonId: String, val totalAmount: String, val currency: String, val participants: List<BackupShareParticipant>, val createdAt: String, val updatedAt: String)
...
data class BackupPlannedMovementShare(val id: String, val expectedMovementId: String, val sourcePlanId: String?, val payerPersonId: String, val mode: String, val payerParts: Int?, val totalAmount: String, val currency: String, val participants: List<BackupPlannedShareParticipant>, val status: String, val materializedTransactionId: String?, val materializedShareId: String?, val createdAt: String, val updatedAt: String)
...
data class SharingBackupSection(val persons: List<BackupSharingPerson>, val movementShares: List<BackupMovementShare>, val recurringPlans: List<BackupRecurringSharePlan>, val plannedShares: List<BackupPlannedMovementShare>) : BackupSection {
    ...
    override fun references() = persons.map { BackupReference.SharingPerson(it.id) } +
        movementShares.flatMap { share -> listOf(BackupReference.MovementShare(share.id)) + share.participants.map { BackupReference.ShareParticipant(it.id) } } +
        ...
        plannedShares.flatMap { share -> listOf(BackupReference.PlannedMovementShare(share.id)) + share.participants.map { BackupReference.PlannedShareParticipant(it.id) } }
}

class SharingBackupSectionExporter(private val personRepository: SharingPersonRepository, private val movementShareRepository: MovementShareRepository, private val recurringPlanRepository: RecurringSharePlanRepository, private val plannedShareRepository: PlannedMovementShareRepository) : BackupSectionExporter {
    override val sectionId = BackupSectionId.SHARING
    override val version = 1
    ...
    override fun export(): SharingBackupSection = SharingBackupSection(
        ...
        plannedShares = plannedShareRepository.listAll().map { share -> BackupPlannedMovementShare(share.id.toString(), share.expectedMovementRef.value, share.sourcePlanId?.toString(), share.payerPersonId.toString(), share.mode.value, share.payerParts, share.totalAmount.toPlainString(), share.currency, share.participants.map { BackupPlannedShareParticipant(it.id.toString(), it.personId.toString(), it.parts, it.amount.toPlainString(), it.reimbursable, it.order) }.sortedBy { it.order }, share.status.name.lowercase(), share.materializedTransactionId, share.materializedShareId?.toString(), share.createdAt.toString(), share.updatedAt.toString()) }.sortedBy { it.id },
    )
}

class SharingBackupSectionImporter(private val personRepository: SharingPersonRepository, private val movementShareRepository: MovementShareRepository, private val recurringPlanRepository: RecurringSharePlanRepository, private val plannedShareRepository: PlannedMovementShareRepository) : BackupSectionImporter {
    override val sectionId = BackupSectionId.SHARING
    ...
    override fun validate(section: BackupSection, context: BackupImportContext): BackupValidationResult {
        ...
        return try {
            ...
            section.movementShares.forEach { share ->
            ...
            section.plannedShares.forEach { share ->
                ...
                PlannedMovementShareStatus.valueOf(share.status.uppercase())
                BigDecimal(share.totalAmount)
                Instant.parse(share.createdAt)
                Instant.parse(share.updatedAt)
                share.participants.forEach { participant ->
                ...
                share.materializedShareId?.let { id -> requireReference(section.movementShares.map { it.id }.toSet(), id, "planned share materialized share") }
            }
    ...
    override fun import(section: BackupSection, context: BackupImportContext) {
        ...
        sharing.movementShares.forEach { share ->
            movementShareRepository.save(MovementShare(MovementShareId.from(share.id), share.sourceTransactionId, SharingPersonId.from(share.payerPersonId), BigDecimal(share.totalAmount), share.currency, share.participants.map { ShareParticipant(ShareParticipantId.from(it.id), SharingPersonId.from(it.personId), BigDecimal(it.amount), it.reimbursable, it.expectedMovementId) }, Instant.parse(share.createdAt), Instant.parse(share.updatedAt)))
        }
        ...
        sharing.plannedShares.forEach { share ->
            plannedShareRepository.save(
                PlannedMovementShare(
                    PlannedMovementShareId(UUID.fromString(share.id)), ExpectedMovementRef(share.expectedMovementId),
                    share.sourcePlanId?.let {
                        RecurringSharePlanId(UUID.fromString(it))
                    },
                    SharingPersonId.from(share.payerPersonId), RecurringShareAllocationMode.from(share.mode), share.payerParts, BigDecimal(share.totalAmount), share.currency, share.participants.map { PlannedMovementShareParticipant(PlannedMovementShareParticipantId(UUID.fromString(it.id)), SharingPersonId.from(it.personId), it.parts, BigDecimal(it.amount), it.reimbursable, it.order) }, PlannedMovementShareStatus.valueOf(share.status.uppercase()), share.materializedTransactionId, share.materializedShareId?.let(MovementShareId::from), Instant.parse(share.createdAt), Instant.parse(share.updatedAt),
                ),
            )
        }
    }
