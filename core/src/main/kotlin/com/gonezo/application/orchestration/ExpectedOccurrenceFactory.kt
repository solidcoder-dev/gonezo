...
data class RecurringOccurrenceSnapshot(val recurringMovementId: String, val occurrenceId: String, val accountId: String, val movementType: String, val amount: BigDecimal, val currency: String, val dueAt: Instant, val description: String?, val merchant: String?, val categoryId: String?, val createdAt: Instant, val items: List<Item>, val tagNames: List<String> = emptyList()) {
    data class Item(val templateItemId: String, val name: String, val amount: BigDecimal)
}
...
class UuidExpectedMovementIdGenerator : ExpectedMovementIdGenerator {
    override fun generate(): ExpectedMovementId = ExpectedMovementId.from(
        java.util.UUID
            .randomUUID()
            .toString(),
    )
...
class UuidExpectedMovementItemIdGenerator : ExpectedMovementItemIdGenerator {
    override fun generate(): String = java.util.UUID
        .randomUUID()
        .toString()
}

class DefaultExpectedOccurrenceFactory(private val expectedMovementIdGenerator: ExpectedMovementIdGenerator = UuidExpectedMovementIdGenerator(), private val expectedMovementItemIdGenerator: ExpectedMovementItemIdGenerator = UuidExpectedMovementItemIdGenerator()) : ExpectedOccurrenceFactory {
