...
class CreateRecurringMovementWorkflow(private val createRecurringMovement: CreateRecurringMovementUC, private val sharePlans: RecurringSharePlanService, private val projectInitialOccurrence: ExpectedOccurrenceProjectionService, private val consistencyBoundary: ConsistencyBoundary = ImmediateConsistencyBoundary) {
    fun execute(command: CreateRecurringMovementCommand, sharingPlan: SharingPlanChange = SharingPlanChange.Keep): RecurringMovementId = consistencyBoundary.withinConsistencyBoundary {
...
class UpdateRecurringMovementWorkflow(private val updateRecurringMovement: UpdateRecurringMovementUC, private val sharePlans: RecurringSharePlanService, private val projectFutureOccurrence: ExpectedOccurrenceProjectionService, private val consistencyBoundary: ConsistencyBoundary = ImmediateConsistencyBoundary) {
    fun execute(command: UpdateRecurringMovementCommand, sharingPlan: SharingPlanChange): RecurringMovementId = consistencyBoundary.withinConsistencyBoundary {
        updateRecurringMovement.execute(command)
        when (sharingPlan) {
            SharingPlanChange.Keep -> {
                Unit
            }

            SharingPlanChange.Remove -> {
                sharePlans.remove(command.recurringMovementId.toString())
            }

            is SharingPlanChange.Replace -> {
                sharePlans.createOrUpdate(
...
class DeactivateRecurringMovementWorkflow(private val deactivateRecurringMovement: DeactivateRecurringMovementUC, private val sharePlans: RecurringSharePlanService, private val consistencyBoundary: ConsistencyBoundary = ImmediateConsistencyBoundary) {
