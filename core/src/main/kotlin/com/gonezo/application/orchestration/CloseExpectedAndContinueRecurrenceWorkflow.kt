...
class DefaultExpectedOccurrenceProjectionService(private val recurringMovementRepository: RecurringMovementRepository, private val occurrenceRepository: RecurringMovementOccurrenceRepository, private val expectedMovementRepository: ExpectedMovementRepository, private val createExpectedMovementUC: CreateExpectedMovementUC, private val expectedOccurrenceFactory: ExpectedOccurrenceFactory = DefaultExpectedOccurrenceFactory(), private val scheduleCalculator: RecurrenceScheduleCalculator = RecurrenceScheduleCalculator(), private val plannedShareInstantiator: PlannedShareInstantiator = NoOpPlannedShareInstantiator) : ExpectedOccurrenceProjectionService {
    override fun projectNext(recurringMovementId: String, projectedAt: Instant): ExpectedMovementId? {
        val movement =
            recurringMovementRepository.findById(
                com.gonezo.recurrence.domain.RecurringMovementId
                    .from(recurringMovementId),
            )
                ?: return null
        ...
        val expectedId =
            existingExpected?.id ?: run {
                val draft =
                    expectedOccurrenceFactory.create(
    ...
    private fun RecurringMovement.requiresExpectedConfirmation(): Boolean = status == com.gonezo.recurrence.domain.RecurringMovementStatus.ACTIVE &&
...
data class CloseExpectedAndContinueRecurrenceCommand(val expectedMovementId: ExpectedMovementId, val action: CloseExpectedAction, val occurredAt: Instant, val transactionId: String? = null)
...
class CloseExpectedAndContinueRecurrenceWorkflow(private val expectedMovementRepository: ExpectedMovementRepository, private val occurrenceRepository: RecurringMovementOccurrenceRepository, private val recurringMovementRepository: RecurringMovementRepository, private val expectedOccurrenceProjectionService: ExpectedOccurrenceProjectionService, private val consistencyBoundary: com.gonezo.application.ConsistencyBoundary) {
    fun execute(command: CloseExpectedAndContinueRecurrenceCommand): ExpectedMovementId? = consistencyBoundary.withinConsistencyBoundary {
        val expected =
            expectedMovementRepository.findById(command.expectedMovementId)
        ...
        val occurrenceId =
            expected.originOccurrenceId?.let(UUID::fromString)
        ...
        val closed =
            when (command.action) {
                ...
                CloseExpectedAction.DISMISS -> {
                    expected.dismiss(command.occurredAt)
                }
            }
