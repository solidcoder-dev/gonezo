...
data class DeactivateRecurringMovementCommand(val recurringMovementId: RecurringMovementId, val deactivatedAt: Instant)

interface DeactivateRecurringMovementUC {
    fun execute(command: DeactivateRecurringMovementCommand)
}

data class ProcessDueRecurringMovementsCommand(val now: Instant, val limit: Int = 100)

data class ProcessDueRecurringMovementsResult(val scanned: Int, val createdOccurrences: Int, val advancedSchedules: Int)
...
data class AcknowledgeRecurringMovementOccurrenceCommand(val occurrenceId: UUID, val status: AcknowledgeRecurringMovementOccurrenceStatus, val ledgerTransactionId: String?, val errorCode: String?, val errorMessage: String?, val acknowledgedAt: Instant)
...
data class PublishRecurrenceOutboxCommand(val limit: Int = 100, val publishedAt: Instant)

data class PublishRecurrenceOutboxResult(val processed: Int, val published: Int, val failed: Int)

interface PublishRecurrenceOutboxUC {
    fun execute(command: PublishRecurrenceOutboxCommand): PublishRecurrenceOutboxResult
}

data class ListRecurringMovementsByAccountQuery(val sourceAccountId: String)
...
data class GetRecurringMovementQuery(val recurringMovementId: RecurringMovementId)
