...
data class RecurringMovement(
    ...
    fun advanceAfterDue(dueAt: Instant, advancedAt: Instant, scheduleCalculator: RecurrenceScheduleCalculator): RecurringMovement {
    ...
    private fun isCompletionByCount(generatedCountAfterAdvance: Int): Boolean = when (val end = recurrenceEnd) {
        is RecurrenceEnd.AfterOccurrences -> generatedCountAfterAdvance >= end.count
        else -> false
    }

    private fun isCompletionByDate(nextDueDate: java.time.LocalDate): Boolean = when (val end = recurrenceEnd) {
