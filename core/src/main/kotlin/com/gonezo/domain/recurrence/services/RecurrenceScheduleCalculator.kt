...
class DailyFrequencyCalculator : FrequencyCalculator {
    ...
    override fun nextDueAt(anchor: ZonedDateTime, previousDueAt: ZonedDateTime, rule: RecurrenceRule): ZonedDateTime = previousDueAt.plusDays(rule.interval.toLong())
}
...
class MonthlyFrequencyCalculator : FrequencyCalculator {
    ...
    private fun resolveMonthlyCandidate(anchor: ZonedDateTime, month: YearMonth, time: LocalTime, zone: ZoneId, rule: RecurrenceRule): ZonedDateTime {
        val localDate =
            when (rule.monthlyPattern) {
...
class RecurrenceScheduleCalculator(private val dailyCalculator: FrequencyCalculator = DailyFrequencyCalculator(), private val weeklyCalculator: FrequencyCalculator = WeeklyFrequencyCalculator(), private val monthlyCalculator: FrequencyCalculator = MonthlyFrequencyCalculator(), private val yearlyCalculator: FrequencyCalculator = YearlyFrequencyCalculator()) {
    ...
    private fun calculatorFor(frequency: RecurrenceFrequency): FrequencyCalculator = when (frequency) {
...
private fun atZonedDateTime(date: LocalDate, time: LocalTime, zone: ZoneId): ZonedDateTime = LocalDateTime.of(date, time).atZone(zone)
