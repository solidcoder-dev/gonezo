...
data class RecurrenceBackupSection(
    val movements: List<BackupRecurringMovement>,
    val occurrences: List<BackupRecurringOccurrence>,
) : BackupSection {
    override val sectionId = BackupSectionId.RECURRENCE
    override val version = 1

    override fun references() = movements.map { BackupReference.RecurringMovement(it.id) }.toSet() + occurrences.map { BackupReference.RecurringOccurrence(it.id) }
}

class RecurrenceBackupSectionExporter(
    private val accountRepository: LedgerAccountRepository,
    private val movementRepository: RecurringMovementRepository,
...
class RecurrenceBackupSectionImporter(
    private val movementRepository: RecurringMovementRepository,
    private val occurrenceRepository: RecurringMovementOccurrenceRepository,
    ...
    override fun validate(section: BackupSection, context: BackupImportContext): BackupValidationResult = try {
        ...
        recurrence.movements.forEach { movement ->
            ...
            RecurringMovementType.from(movement.type); RecurrenceFrequency.from(movement.rule.frequency); MonthlyPattern.from(movement.rule.monthlyPattern); RecurringMovementStatus.from(movement.status); RecurringMovementReviewPolicy.from(movement.reviewPolicy)
            BigDecimal(movement.amount); Instant.parse(movement.startAt); movement.nextDueAt?.let { Instant.parse(it) }
        }
        recurrence.occurrences.forEach { occurrence -> requireReference(movementIds, occurrence.recurringMovementId, "occurrence recurring movement"); occurrence.ledgerTransactionId?.let { requireContext(context.validationContext.containsMovement(it), "occurrence ledger transaction", it) }; RecurringMovementOccurrenceStatus.from(occurrence.status); Instant.parse(occurrence.dueAt); Instant.parse(occurrence.createdAt); Instant.parse(occurrence.updatedAt) }
        uniqueIds(recurrence.occurrences.map { it.id }, "recurrence occurrence")
        BackupValidationResult.Valid
    } catch (error: IllegalArgumentException) { BackupValidationResult.Invalid(if (error is BackupReferenceValidationException) BackupErrorCode.INVALID_REFERENCE else BackupErrorCode.INVALID_DATA, error.message ?: "Invalid recurrence backup section") }
    override fun import(section: BackupSection, context: BackupImportContext) {
        ...
        recurrence.movements.forEach { value ->
            val end = when (value.recurrenceEnd.kind) { "never" -> com.gonezo.recurrence.domain.RecurrenceEnd.Never; "on_date" -> com.gonezo.recurrence.domain.RecurrenceEnd.OnDate(LocalDate.parse(value.recurrenceEnd.date!!)); "after_occurrences" -> com.gonezo.recurrence.domain.RecurrenceEnd.AfterOccurrences(value.recurrenceEnd.count!!) ; else -> error("Unsupported recurrence end") }
            ...
            movementRepository.save(RecurringMovement(RecurringMovementId.from(value.id), RecurringMovementType.from(value.type), value.sourceAccountId, value.targetAccountId, BigDecimal(value.amount), value.currency, value.destinationAmount?.let(::BigDecimal), value.destinationCurrency, value.exchangeRate?.let(::BigDecimal), value.description, value.merchant, value.categoryId, RecurringMovementReviewPolicy.from(value.reviewPolicy), value.splitItems.map { RecurringMovement.SplitItem(it.id, it.name, BigDecimal(it.amount)) }, rule, end, Instant.parse(value.startAt), value.zoneId, value.nextDueAt?.let(Instant::parse), RecurringMovementStatus.from(value.status), value.generatedOccurrences, Instant.parse(value.createdAt), Instant.parse(value.updatedAt), value.deactivatedAt?.let(Instant::parse), value.completedAt?.let(Instant::parse), value.tagNames))
        }
