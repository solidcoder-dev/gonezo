...
import com.gonezo.application.orchestration.backup.BackupRecurringSharePlan
import com.gonezo.application.orchestration.backup.BackupShareParticipant
...
import com.gonezo.expected.application.backup.BackupExpectedMovement
import com.gonezo.expected.application.backup.BackupExpectedSplitItem
import com.gonezo.expected.application.backup.ExpectedBackupSection
import com.gonezo.recurrence.application.backup.BackupRecurrenceEnd
import com.gonezo.recurrence.application.backup.BackupRecurrenceRule
import com.gonezo.recurrence.application.backup.BackupRecurringMovement
import com.gonezo.recurrence.application.backup.BackupRecurringOccurrence
import com.gonezo.recurrence.application.backup.BackupRecurringSplitItem
...
private fun plannedShare(v: BackupPlannedExpenseShare) = JSONObject().put("id", v.id).put("expectedMovementId", v.expectedMovementId).putNullable("sourcePlanId", v.sourcePlanId).put("payerPersonId", v.payerPersonId).put("mode", v.mode).putNullable("payerParts", v.payerParts).put("totalAmount", v.totalAmount).put("currency", v.currency).put(
    ...
    JSONArray(
        v.participants.map {
            JSONObject().put("id", it.id).put("personId", it.personId).putNullable("parts", it.parts).put("amount", it.amount).put("reimbursable", it.reimbursable).put("order", it.order)
        },
    ),
