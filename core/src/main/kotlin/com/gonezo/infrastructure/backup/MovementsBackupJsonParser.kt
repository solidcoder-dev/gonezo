...
class MovementsBackupJsonParser {
    fun parse(json: String): MovementsBackupSnapshot {
        ...
        return MovementsBackupSnapshot(
            ...
            accounts =
            root.optJSONArray("accounts").orEmpty().mapObjects { item ->
                BackupAccount(
            ...
            categories =
            root.optJSONArray("categories").orEmpty().mapObjects { item ->
                BackupCategory(
                    id = item.getString("id"),
                    name = item.getString("name"),
            ...
            tags =
            root.optJSONArray("tags").orEmpty().mapObjects { item ->
                BackupTag(
                    id = item.getString("id"),
                    name = item.getString("name"),
                    status = item.optString("status", "active"),
            ...
            postedMovements =
            root.optJSONArray("postedMovements").orEmpty().mapObjects { item ->
                ...
                BackupPostedMovement(
                    ...
                    splitItems =
                    item.optJSONArray("splitItems").orEmpty().mapObjects { split ->
                        BackupSplitItem(
    ...
    private fun JSONArray.mapStrings(): List<String> = (0 until length()).mapNotNull { index -> optString(index).trim().ifBlank { null } }
