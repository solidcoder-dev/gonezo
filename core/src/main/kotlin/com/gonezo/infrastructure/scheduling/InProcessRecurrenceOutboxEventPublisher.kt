...
class InProcessRecurrenceOutboxEventPublisher(private val handleRecurringMovementDueForExpectedUC: HandleRecurringMovementDueForExpectedUC, private val clock: Clock = Clock.systemUTC()) : RecurrenceOutboxEventPublisher {
        ...
        when (message.eventType) {
            RecurringMovementDueIntegrationEvent.EVENT_TYPE -> {
                ...
                when (RecurringMovementReviewPolicy.from(event.reviewPolicy)) {
                    RecurringMovementReviewPolicy.AUTOMATIC -> {
                        Unit
                    }
                ...
                throw IllegalArgumentException("Unsupported recurrence outbox event type: ${message.eventType}")
            }
        }
    }
}
