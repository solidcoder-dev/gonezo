...
data class RecurrenceSchedulerRunResult(val dueProcessing: ProcessDueRecurringMovementsResult, val outboxPublishing: PublishRecurrenceOutboxResult)

class RecurrenceSchedulerJob(private val processDueRecurringMovementsUC: ProcessDueRecurringMovementsUC, private val publishRecurrenceOutboxUC: PublishRecurrenceOutboxUC, private val clock: Clock = Clock.systemUTC()) {
    fun run(batchSize: Int = 100): RecurrenceSchedulerRunResult {
        require(batchSize > 0) { "batchSize must be greater than 0" }
        val now = clock.instant()
        val dueProcessing =
            processDueRecurringMovementsUC.execute(
                ProcessDueRecurringMovementsCommand(
                    now = now,
                    limit = batchSize,
                ),
            )
        val outboxPublishing =
            publishRecurrenceOutboxUC.execute(
