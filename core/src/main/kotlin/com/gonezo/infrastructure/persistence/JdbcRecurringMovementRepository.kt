...
@Repository
class JdbcRecurringMovementRepository(private val jdbcTemplate: NamedParameterJdbcTemplate) : RecurringMovementRepository {
    ...
    override fun findById(id: RecurringMovementId): RecurringMovement? {
        val sql =
            ...
            """.trimIndent()
        return jdbcTemplate
            .query(sql, MapSqlParameterSource("id", id.toString()), baseRowMapper())
            .firstOrNull()
            ?.toMovement()
    }
    ...
    override fun findDue(now: Instant, limit: Int): List<RecurringMovement> {
        ...
        return jdbcTemplate
            .query(sql, params, baseRowMapper())
            .map { it.toMovement() }
    }
    ...
    private fun baseRowMapper(): RowMapper<RecurringMovementRow> = RowMapper { rs: ResultSet, _ ->
        ...
        val monthlyWeekday =
            rs
                .getString("rule_monthly_weekday")
                ?.trim()
    ...
    private fun movementParams(movement: RecurringMovement): MapSqlParameterSource {
        val (endKind, endOnDate, endAfterOccurrences) =
            when (val end = movement.recurrenceEnd) {
