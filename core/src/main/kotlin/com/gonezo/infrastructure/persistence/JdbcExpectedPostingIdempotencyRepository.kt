...
class JdbcExpectedPostingIdempotencyRepository(private val jdbc: NamedParameterJdbcTemplate) : ExpectedPostingIdempotencyRepository {
    override fun findByKey(idempotencyKey: String): ProcessedExpectedPosting? = jdbc
        .query(
            """
            ...
            ProcessedExpectedPosting(
                ...
                result =
                PostExpectedMovementResult(
                    rs.getString("transaction_id"),
                    rs.getString("share_id"),
                    rs.getString("next_expected_movement_id"),
                ),
                completionStatus = rs.getString("completion_status"),
            )
        }.firstOrNull()

    override fun findByExpectedMovementId(expectedMovementId: String): ProcessedExpectedPosting? = jdbc
        .query(
            """
                select idempotency_key, expected_movement_id, transaction_id, share_id, next_expected_movement_id, completion_status
                from expected_posting_attempts
