...
class JdbcAnalyticsExclusionRepository(private val jdbcTemplate: NamedParameterJdbcTemplate) : AnalyticsExclusionRepository {
    ...
    override fun findByScope(scopeType: AnalyticsExclusionScopeType, scopeId: String): List<AnalyticsExclusion> = jdbcTemplate.query(
        """
    ...
    override fun listAll(): List<AnalyticsExclusion> = jdbcTemplate.query(
        """
            select id, scope_type, scope_id, reason, created_at
            from analytics_exclusions
            order by created_at asc, id asc
