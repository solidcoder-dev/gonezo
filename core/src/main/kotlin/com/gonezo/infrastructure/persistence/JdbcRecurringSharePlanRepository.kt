...
class JdbcRecurringSharePlanRepository(private val jdbc: NamedParameterJdbcTemplate) : RecurringSharePlanRepository {
        ...
        plan.participants.forEach { participant ->
            jdbc.update(
                ...
                MapSqlParameterSource()
                    ...
                    .addValue("plan_id", plan.id.toString())
                    .addValue("person_id", participant.personId.toString())
                    .addValue("parts", participant.parts)
                    .addValue("amount", participant.fixedAmount?.toPlainString())
    ...
    override fun findByRecurringMovementRef(ref: RecurringMovementRef): RecurringSharePlan? = find("p.recurring_movement_ref = :movement", MapSqlParameterSource("movement", ref.value))
                ...
                val participants =
                    jdbc.query(
                        ...
                        RecurringShareParticipantTemplate(
                            RecurringShareParticipantTemplateId(java.util.UUID.fromString(child.getString("id"))),
                            SharingPersonId.from(child.getString("person_id")),
                            child.getObject("participant_parts")?.toString()?.toInt(),
                            child.getString("fixed_amount")?.let(::BigDecimal),
                            child.getInt("reimbursable") == 1,
                            child.getInt("participant_order"),
                        )
                    }
                RecurringSharePlan(
                    planId,
                    RecurringMovementRef(rs.getString("recurring_movement_ref")),
                    SharingPersonId.from(rs.getString("payer_person_id")),
                    RecurringShareAllocationMode.from(rs.getString("mode")),
                    rs.getString("currency"),
                    rs.getObject("payer_parts")?.toString()?.toInt(),
                    participants,
    ...
    private fun planParams(plan: RecurringSharePlan) = MapSqlParameterSource()
        .addValue(
            "id",
            plan.id.toString(),
        ).addValue("movement", plan.recurringMovementRef.value)
        .addValue("payer", plan.payerPersonId.toString())
