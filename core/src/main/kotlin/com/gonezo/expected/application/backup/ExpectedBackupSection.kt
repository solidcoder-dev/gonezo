...
data class ExpectedBackupSection(val movements: List<BackupExpectedMovement>) : BackupSection {
    ...
    override fun references() = movements.flatMap { movement -> listOf(BackupReference.ExpectedMovement(movement.id)) + movement.splitItems.map { BackupReference.ExpectedSplitItem(it.id) } }
}

class ExpectedBackupSectionExporter(private val accountRepository: LedgerAccountRepository, private val movementRepository: ExpectedMovementRepository) : BackupSectionExporter {
    override val sectionId = BackupSectionId.EXPECTED
    override val version = 1
    ...
    private fun movement(value: ExpectedMovement) = BackupExpectedMovement(
        ...
        dismissedAt = value.dismissedAt?.toString(), tagNames = value.tagNames.sorted(), tagIds = value.tagIds.sorted(),
    )
}
...
class ExpectedBackupSectionImporter(private val repository: ExpectedMovementRepository) : BackupSectionImporter {
    ...
    override fun validate(section: BackupSection, context: BackupImportContext): BackupValidationResult = try {
        ...
        expected.movements.forEach { movement ->
            uniqueIds(movement.splitItems.map { it.id }, "expected split item")
            ...
            movement.splitItems.forEach { item ->
                item.sourceTemplateItemId?.let { source ->
                    if (movement.originRecurringMovementId == null || !context.validationContext.containsRecurringSplitItem(source, movement.originRecurringMovementId)) {
                        throw BackupReferenceValidationException("Invalid expected source template item reference: $source")
                    }
                }
            }
            ExpectedMovementType.from(movement.type)
            ExpectedMovementStatus.from(movement.status)
            BigDecimal(movement.amount)
            Instant.parse(movement.expectedAt)
            Instant.parse(movement.createdAt)
            Instant.parse(movement.updatedAt)
        }
        BackupValidationResult.Valid
    } catch (error: IllegalArgumentException) {
        BackupValidationResult.Invalid(if (error is BackupReferenceValidationException) BackupErrorCode.INVALID_REFERENCE else BackupErrorCode.INVALID_DATA, error.message ?: "Invalid expected backup section")
    }
