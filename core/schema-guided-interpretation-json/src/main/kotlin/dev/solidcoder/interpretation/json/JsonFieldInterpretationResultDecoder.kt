...
class JsonFieldInterpretationResultDecoder(private val outputNormalizer: JsonFieldOutputNormalizer = JsonFieldOutputNormalizer()) : FieldInterpretationResultDecoder {
    ...
    private fun requireDecimal(rawValue: Any?): BigDecimal = when (rawValue) {
        is String,
        is Number,
        -> parseBigDecimal(rawValue.toString(), "decimal")
    ...
    private fun decodingFailure(violation: FieldOutputViolation, message: String, cause: Throwable? = null): FieldOutputDecodingException = FieldOutputDecodingException(violation, message, cause)
}
