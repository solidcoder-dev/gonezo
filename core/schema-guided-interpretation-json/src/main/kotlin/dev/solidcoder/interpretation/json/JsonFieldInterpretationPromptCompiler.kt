...
class JsonFieldInterpretationPromptCompiler : FieldInterpretationPromptCompiler {
    override fun compile(request: InterpretationRequest, field: FieldSpec, variant: FieldPromptVariant, previousViolation: FieldOutputViolation?): StructuredGenerationRequest = StructuredGenerationRequest(
        prompt = when (variant) {
    ...
    private fun retryPrompt(request: InterpretationRequest, field: FieldSpec, previousViolation: FieldOutputViolation): String = buildString {
    ...
    private fun StringBuilder.appendFieldHeader(request: InterpretationRequest, field: FieldSpec, includeInput: Boolean, includeDescription: Boolean, includeRequired: Boolean) {
    ...
    private fun contextLine(type: FieldType, request: InterpretationRequest): String = when (type) {
        ...
        FieldType.BOOLEAN,
        FieldType.INTEGER,
        -> "context: inputLanguage=${request.inputLanguage}"
    }
    ...
    private fun allowedValues(field: FieldSpec): String = buildString {
