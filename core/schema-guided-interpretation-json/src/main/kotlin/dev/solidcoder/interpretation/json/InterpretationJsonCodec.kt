    ...
    private fun decodeInterpretation(json: JSONObject): FieldInterpretation {
        ...
        return when (kind) {
            ...
            "ambiguous" -> {
                ...
                json.getJSONArray("candidates").let { values ->
                    FieldInterpretation.Ambiguous(buildList(values.length()) { for (index in 0 until values.length()) add(decodeCandidate(values.getJSONObject(index))) })
                }
            }
