...
class SpeechTranscriptionContractTest {
    ...
    fun `rejects incompatible language options and definitive success issues`() {
        ...
        assertThatThrownBy {
            TranscriptionRequest(AudioSourceRef.of("capture-1"), language = "es", detectLanguageAutomatically = true)
        }.isInstanceOf(IllegalArgumentException::class.java)
        assertThatThrownBy {
            TranscriptionResult(Transcript("hello"), listOf(TranscriptionIssue("native", "failed", TranscriptionIssueSeverity.DEFINITIVE)))
        }.isInstanceOf(IllegalArgumentException::class.java)
    }
