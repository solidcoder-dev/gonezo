...
data class FieldResult(val key: FieldKey, val interpretation: FieldInterpretation)

@ConsistentCopyVisibility
data class InterpretationResult private constructor(val specId: InterpretationSpecId, val specVersion: InterpretationSpecVersion, val fields: List<FieldResult>, val issues: List<InterpretationIssue> = emptyList()) {
    companion object {
        fun fromDecoded(specId: InterpretationSpecId, specVersion: InterpretationSpecVersion, fields: List<FieldResult>, issues: List<InterpretationIssue> = emptyList()): InterpretationResult = InterpretationResult(
            specId = specId,
        ...
        fun forSpec(spec: InterpretationSpec, candidates: Map<String, FieldCandidate>, issues: List<InterpretationIssue> = emptyList()): InterpretationResult {
            val unknownKeys =
                candidates.keys.filterNot { candidateKey ->
        ...
        fun forSpec(spec: InterpretationSpec, fields: List<FieldResult>, issues: List<InterpretationIssue> = emptyList()): InterpretationResult {
            ...
            fields.forEach { field ->
                val fieldSpec =
                    requireNotNull(fieldSpecsByKey[field.key]) {
        ...
        private fun validateCompatibility(fieldSpec: FieldSpec, interpretation: FieldInterpretation) {
            when (interpretation) {
