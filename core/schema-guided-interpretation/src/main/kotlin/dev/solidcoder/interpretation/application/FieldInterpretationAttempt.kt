...
data class FieldInterpretationAttempt(val fieldKey: FieldKey, val fieldIndex: Int, val attemptNumber: Int, val promptVariant: FieldPromptVariant, val status: FieldInterpretationAttemptStatus, val durationMs: Long, val outputLength: Int? = null, val raw: String? = null, val failureCode: InterpretationFailureCode? = null, val phase: StructuredGenerationFailurePhase? = null) {
    init {
        ...
        when (status) {
            ...
            -> require(raw != null) { "raw output is required for decoded attempts" }
