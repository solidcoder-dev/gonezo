...
class OnDeviceInputInterpreter(private val promptCompiler: FieldInterpretationPromptCompiler, private val runtime: StructuredGenerationRuntime, private val resultDecoder: FieldInterpretationResultDecoder, private val fieldProcessingOrder: FieldProcessingOrder = SpecFieldProcessingOrder, private val interpretationClock: InterpretationClock = SystemInterpretationClock, private val interpretationTimeoutMs: Long = GLOBAL_INTERPRETATION_TIMEOUT_MS) : InputInterpreter {
    override suspend fun interpret(request: InterpretationRequest): InterpretationOutcome {
        ...
        val originalIndexByKey =
            request.spec.fields.withIndex().associate { (index, field) ->
                field.key to index
            }
        ...
        for (field in orderedFields) {
            ...
            if (isInterpretationTimedOut(interpretationStartedAt)) {
                return timeoutFailure(
                    ...
                    cause =
                    StructuredGenerationTimeoutException(
            ...
            when (primaryResult) {
                ...
                is FieldInterpretationOutcome.DecodingFailure -> {
                    ...
                    when (retryResult) {
                        ...
                        is FieldInterpretationOutcome.Cancelled -> {
                            throw InterpretationCancellationException(
                                attempts = attempts.toList() + retryResult.attempts,
                                cause = retryResult.cause,
                            )
                        }
    ...
    private suspend fun executeAttempt(request: InterpretationRequest, field: FieldSpec, originalFieldIndex: Int, attemptNumber: Int, promptVariant: FieldPromptVariant, previousViolation: FieldOutputViolation?, interpretationStartedAt: Long): FieldInterpretationOutcome {
    ...
    private fun compilePrompt(request: InterpretationRequest, field: FieldSpec, variant: FieldPromptVariant, previousViolation: FieldOutputViolation? = null): StructuredGenerationRequest = promptCompiler.compile(request, field, variant, previousViolation)

    private fun remainingInterpretationBudgetMs(interpretationStartedAt: Long): Long = interpretationTimeoutMs - (interpretationClock.nowMillis() - interpretationStartedAt)

    private fun isInterpretationTimedOut(interpretationStartedAt: Long): Boolean = remainingInterpretationBudgetMs(interpretationStartedAt) <= 0

    private fun timeoutFailure(field: FieldSpec?, fieldIndex: Int?, fieldCount: Int, interpretationStartedAt: Long, completedFieldKeys: List<FieldKey>, attempts: List<FieldInterpretationAttempt>, cause: StructuredGenerationTimeoutException): InterpretationOutcome.Failure = failure(
        code = InterpretationFailureCode.INFERENCE_FAILED,
        message = "The local interpreter timed out before finishing all fields.",
        recoverable = true,
        cause = cause,
        diagnostics =
        InterpretationFailureDiagnostics(
            fieldKey = field?.key,
    ...
    private fun failure(code: InterpretationFailureCode, message: String, recoverable: Boolean, cause: Throwable, diagnostics: InterpretationFailureDiagnostics, attempts: List<FieldInterpretationAttempt>): InterpretationOutcome.Failure = InterpretationOutcome.Failure(
    ...
    private fun diagnostics(field: FieldSpec, fieldIndex: Int, fieldCount: Int, startedAt: Long, completedFieldKeys: List<FieldKey>, failureCode: InterpretationFailureCode, phase: StructuredGenerationFailurePhase, outputLength: Int? = null): InterpretationFailureDiagnostics = InterpretationFailureDiagnostics(
        fieldKey = field.key,
        fieldIndex = fieldIndex,
        fieldCount = fieldCount,
    ...
    private fun generationFailureAttempt(field: FieldSpec, fieldIndex: Int, startedAt: Long, attemptNumber: Int, promptVariant: FieldPromptVariant, failureCode: InterpretationFailureCode, phase: StructuredGenerationFailurePhase): FieldInterpretationAttempt = FieldInterpretationAttempt(
        fieldKey = field.key,
        fieldIndex = fieldIndex,
        attemptNumber = attemptNumber,
        promptVariant = promptVariant,
    ...
    private fun cancelledAttempt(field: FieldSpec, fieldIndex: Int, startedAt: Long, attemptNumber: Int, promptVariant: FieldPromptVariant): FieldInterpretationAttempt = FieldInterpretationAttempt(
        fieldKey = field.key,
        fieldIndex = fieldIndex,
        attemptNumber = attemptNumber,
        promptVariant = promptVariant,
    ...
    private sealed interface FieldInterpretationOutcome {
        ...
        data class GenerationFailure(val failureCode: InterpretationFailureCode, val message: String, val recoverable: Boolean, val cause: Throwable, val phase: StructuredGenerationFailurePhase, val outputLength: Int?, val startedAt: Long, val attempts: List<FieldInterpretationAttempt>) : FieldInterpretationOutcome

        data class Cancelled(val attempts: List<FieldInterpretationAttempt>, val cause: CancellationException) : FieldInterpretationOutcome
    }
}
