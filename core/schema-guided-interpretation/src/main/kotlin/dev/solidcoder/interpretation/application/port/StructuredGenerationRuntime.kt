package dev.solidcoder.interpretation.application.port.generation
...
data class StructuredGenerationRequest(val prompt: String, val spec: InterpretationSpec, val fieldKey: String? = null, val fieldIndex: Int? = null, val attemptNumber: Int? = null, val promptVariant: FieldPromptVariant = FieldPromptVariant.PRIMARY, val generationTimeoutMs: Long? = null) {
    init {
...
data class StructuredGenerationResult(val output: String)
...
open class StructuredGenerationException(val failureCode: InterpretationFailureCode, val recoverable: Boolean, val phase: StructuredGenerationFailurePhase? = null, message: String, cause: Throwable? = null) : RuntimeException(message, cause)
...
class StructuredGenerationTimeoutException(val timeoutKind: StructuredGenerationTimeoutKind, message: String, cause: Throwable? = null) :
...
interface FieldInterpretationPromptCompiler {
    fun compile(request: InterpretationRequest, field: FieldSpec, variant: FieldPromptVariant = FieldPromptVariant.PRIMARY, previousViolation: FieldOutputViolation? = null): StructuredGenerationRequest
}
