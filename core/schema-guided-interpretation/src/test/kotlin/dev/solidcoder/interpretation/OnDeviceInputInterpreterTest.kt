...
```""",
        ...
        assertThat(outcome.attempts[2].raw).isEqualTo(
            """```json
{"kind":"resolved","value":"2026-07-14","confidence":0.91}
```""",
        )
    ...
    fun `records cancelled output when decoding is interrupted after generation`() = runBlocking {
        ...
        val interpreter = OnDeviceInputInterpreter(
            ...
            resultDecoder = object : FieldInterpretationResultDecoder {
                override fun decode(field: FieldSpec, generationResult: StructuredGenerationResult): FieldInterpretation = throw CancellationException("cancelled while decoding")
            },
        )
    ...
    private class MutableClock(private var nowMs: Long = 0L) : InterpretationClock {
    ...
    private class RecordingPromptCompiler : FieldInterpretationPromptCompiler {
        ...
        override fun compile(request: InterpretationRequest, field: FieldSpec, variant: FieldPromptVariant, previousViolation: FieldOutputViolation?): StructuredGenerationRequest {
            fields += field
    ...
    private class ScriptedRuntime(private val outputsByFieldKey: Map<String, String>, private val outputsByFieldKeySequence: Map<String, List<String>> = emptyMap(), private val failureByFieldKey: Map<String, StructuredGenerationException> = emptyMap(), private val onGenerate: suspend (String) -> Unit = {}) : StructuredGenerationRuntime {
    ...
    private class ScriptedDecoder(private val interpretationsByFieldKey: Map<String, FieldInterpretation>) : FieldInterpretationResultDecoder {
        override fun decode(field: FieldSpec, generationResult: StructuredGenerationResult): FieldInterpretation {
