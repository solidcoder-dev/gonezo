...
class MacroAnalyticsPublicationWireV7Parser : MacroAnalyticsPublicationPayloadParser {
    override fun parse(json: String): ValidatedMacroAnalyticsPublication {
        ...
        val tagUsage = contribution.parseTagUsageContribution(financial.currencies)
        return ValidatedMacroAnalyticsPublication(
            ProtocolVersion(7),
            ContributorId(contributorId),
            AnalyticsPeriod(period),
            PublicationRevision(revision),
            MacroAnalyticsContribution(SchemaVersion(7), dimensions, financial, categories, recurring, sharing, merchants, balances, tagUsage),
        )
    }
